...
/**
 * Servlet implementation class FindInPatient
 */
public class FindInPatient extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public FindInPatient() {
        super();
...
PrintWriter out = response.getWriter();
		...
		try {
			In_Patient_Student ips = InOutPatientDAO.FindRegUsr(request.getParameter("opno"));
			...
			if(ips!=null)
			{
	            ...
	            session.setAttribute("FindInPatientStudentData", ips); 
	            response.sendRedirect("ShowInPatientStudent.jsp");
			}
			else
			{
